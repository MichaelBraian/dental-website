  ...
  return (
    <div className="container py-12 md:py-24">
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {dictionary.items &&
          dictionary.items.map((tech: any, index: number) => (
            <TechCard key={index} tech={tech} icon={techIcons[index % techIcons.length]} locale={locale} />
          ))}
      </div>
    </div>
  )
...
function TechCard({ tech, icon, locale }: { tech: any; icon: React.ReactNode; locale: string }) {
